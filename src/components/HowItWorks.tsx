...
export function HowItWorks() {
  return (
    <section id="how-it-works" className="py-16 xs:py-20 sm:py-24 md:py-32 px-4 sm:px-6 bg-white relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 xs:gap-5 sm:gap-6 md:gap-8">
          {steps.map((step, index) => (
            ...
            >
              ...
              <div className="relative h-full p-5 xs:p-6 sm:p-8 rounded-xl xs:rounded-2xl bg-gradient-to-br from-gray-50 to-white border border-gray-200/50 shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden">
                ...
                {/* Step number - Mobile-first sizing */}
                <div className="absolute top-3 right-3 xs:top-4 xs:right-4 text-4xl xs:text-5xl sm:text-6xl font-bold text-gray-100 group-hover:text-gray-200 transition-colors">
                  {step.number}
                </div>

                {/* Icon - Mobile-first sizing */}
                <div className={`relative mb-4 xs:mb-5 sm:mb-6 inline-flex p-3 xs:p-3.5 sm:p-4 rounded-xl xs:rounded-2xl bg-gradient-to-br ${step.gradient} text-white group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
                  {step.icon}
                </div>

                {/* Title - Mobile-first typography */}
                <h3 className="text-base xs:text-lg sm:text-xl font-bold text-gray-900 mb-2 sm:mb-3 group-hover:text-accent-600 transition-colors relative z-10">
                  {step.title}
                </h3>

                {/* Description - Mobile-first typography */}
                <p className="text-sm xs:text-base text-gray-600 leading-relaxed relative z-10">{step.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
