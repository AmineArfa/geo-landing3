...
export function LLMVisibility() {
  ...
  return (
    <section id="llm-visibility" className="py-20 sm:py-24 md:py-32 px-4 sm:px-6 bg-gradient-to-b from-gray-50/50 to-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-accent-50 to-purple-50 border border-accent-200/50 mb-6">
            ...
            <span className="text-sm font-semibold text-accent-700">LLM Visibility</span>
          </div>
        ...
        >
          <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-6 sm:mb-8 text-center px-4">Your Ranking Across LLM Platforms</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {llmPlatforms.map((platform, index) => (
              <motion.div
                ...
                className={`relative p-4 sm:p-6 bg-white rounded-xl sm:rounded-2xl border-2 cursor-pointer transition-all ${
                  selectedPlatform === platform.name 
                    ? 'border-accent-500 shadow-xl' 
                    : 'border-gray-200 hover:border-gray-300 shadow-lg'
                }`}
              >
                <div className={`absolute top-0 right-0 w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br ${platform.color} rounded-bl-xl sm:rounded-bl-2xl flex items-center justify-center text-2xl sm:text-3xl opacity-20`}>
                  {platform.icon}
                </div>
                <div className="relative z-10">
                  <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                    <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-gradient-to-br ${platform.color} flex items-center justify-center text-xl sm:text-2xl shadow-lg`}>
                      {platform.icon}
                    </div>
                    <div>
                  ...
                  <div className="flex items-end gap-2">
                    <span className="text-3xl sm:text-4xl font-bold text-gray-900">#{platform.rank}</span>
                    <span className="text-green-600 font-semibold text-xs sm:text-sm mb-1">{platform.change}</span>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-gray-500 font-medium">Live monitoring</span>
          </div>
            ...
            {sampleQueries.map((query, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="group p-3 bg-white rounded-lg border border-gray-200/60 hover:border-gray-300 hover:shadow-md transition-all cursor-pointer"
                >
                  {/* Query Header */}
                  <div className="flex items-start gap-2 mb-3">
                    <div className="w-4 h-4 rounded bg-gradient-to-br from-accent-500 to-purple-500 flex items-center justify-center text-white font-bold text-[9px] flex-shrink-0 mt-0.5">
                      Q
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-gray-900 text-xs leading-tight mb-1.5">{query.query}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          {[
            ...
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ delay: index * 0.1 }}
              className="p-5 sm:p-6 bg-white rounded-lg sm:rounded-xl border border-gray-200 shadow-lg"
            >
